...
import FormContentInventory from '@/Content/FormContentInventory';
...
export default function IndexSales(props) {
    ...
    const inventoryData = props.dataInventory;

    const filteredInventory = inventoryData.filter((inventory) =>
        inventory.in_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        inventory.in_date.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const handleEdit = (e) => {
        e.preventDefault();
        put(`/inventory/${openModal.dataInventory.in_id}`, {
            onSuccess: () => {
        ...
        <AuthenticatedLayout
            header={
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <button onClick={() => setOpenModal(prev => ({ ...prev, isOpen: true, action: 'TAMBAH', dataInventory: '', title: 'Tambah Data Inventory' }))}
                                ...
                                <tbody className="divide-y divide-gray-100 text-sm text-gray-600">
                                    {filteredInventory.length > 0 ? (
                                        filteredInventory.map((inventory) => (
                                            <tr key={inventory.in_id} className="hover:bg-gray-50/50 transition-colors">
                                                <td className="px-6 py-4 font-medium text-gray-400 text-center">{inventory.in_id}</td>
                                                <td className="px-6 py-4 font-semibold text-gray-900">{inventory.in_name}</td>
                                                <td className="px-6 py-4 text-gray-500">{inventory.in_date}</td>
                                                <td className="px-6 py-4">{inventory.in_harga_beli}</td>
                                                <td className="px-6 py-4">{inventory.in_stock}</td>
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${inventory.in_status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                                        {inventory.in_status}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex items-center justify-center gap-2">
                                                        <button onClick={() => setOpenModal(prev => ({ ...prev, isOpen: true, action: 'DETAIL', dataInventory: inventory, title: 'Detail Data Inventory' }))}
                                                        ...
                                                        <button onClick={() => setOpenModal(prev => ({ ...prev, isOpen: true, action: 'EDIT', dataInventory: inventory, title: 'Edit Data Inventory' }))}
                                                        ...
                                                        <button onClick={() => handleDelete(inventory.in_id)}
            ...
            {openModal.action === 'TAMBAH' &&
                <ModalView openModal={openModal.isOpen} data={openModal.data} closeModal={() => closeModal()}>
                    <form onSubmit={handleTambah}>
                        <FormContentInventory action={openModal.action} setData={setData} data={data} errors={errors} title={openModal.action} defaultValue={openModal.dataInventory} />
                    </form>
                </ModalView>
            }
            {openModal.action === 'EDIT' &&
                <ModalView openModal={openModal.isOpen} data={openModal.data} closeModal={() => closeModal()}>
                    <form onSubmit={handleEdit}>
                        <FormContentInventory title={openModal.action} data={data} setData={setData} defaultValue={openModal.dataInventory} action={openModal.action} errors={errors} />
                    </form>
                </ModalView>
            }
